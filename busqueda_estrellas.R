...
# Carga del catalogo Hipparcos  // No tenemos clasificacion
...
#numero de intervalos (igual sobre RA y DE) USA LAS COLUMNSA J2000 POR LA DIFERENCIA EN TIEMPO DE CLASIFICACION
...
# Nuestro vecindario estara delimitado por un cuadrado 3x3
vecx <- c(-1,0,1)
vecy <- c(-1,0,1)
ind.estrella <- data.frame(matrix(nrow=nrow(hip), ncol=2))
names(ind.estrella) <- c("rec","dec")                              #A que hace referencia REC /RA/ y DEC /DE/ ?
...
#24 ID Cruzadas 
nueva = symbad.aumentado[ is.na(symbad.aumentado$idcruzHip) == FALSE, ]

min.dist = min(nueva$dist)
max.dist = max(nueva$dist)



nueva2 = symbad.aumentado[ symbad.aumentado$dist > min.dist & symbad.aumentado$dist < max.dist, ]
